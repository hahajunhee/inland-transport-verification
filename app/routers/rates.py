...
def list_rates(
    ...
    return rate_service.get_all_rates(db, charge_type, pickup_code, dest_code)


@router.post("", response_model=RateResponse)
def create_rate(body: RateCreate, db: Session = Depends(get_db)):
    return rate_service.create_rate(db, body.model_dump())


@router.put("/{rate_id}", response_model=RateResponse)
def update_rate(rate_id: int, body: RateUpdate, db: Session = Depends(get_db)):
    rate = rate_service.update_rate(db, rate_id, body.model_dump(exclude_unset=True))
    if not rate:
...
def delete_rate(rate_id: int, db: Session = Depends(get_db)):
    ok = rate_service.delete_rate(db, rate_id)
    if not ok:
        raise HTTPException(status_code=404, detail="요율을 찾을 수 없습니다.")
    return {"ok": True}
